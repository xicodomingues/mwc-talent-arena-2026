# MWC Talent Arena 2026 – Schedule Viewer

A single-page schedule viewer for [MWC Talent Arena 2026](https://talentarena.tech/) (March 2-4, Barcelona).

Three views: **List**, **Calendar** (vertical grid), and **Timeline** (horizontal Gantt). Filter by day, stage, topic, and language. Search, hide sessions, and track what's happening now.

**Live:** https://xicodomingues.github.io/mwc-talent-arena-2026/

## Vibecoded

This project was vibecoded with [Claude](https://claude.ai) (Opus) using [Claude Code](https://docs.anthropic.com/en/docs/claude-code).
